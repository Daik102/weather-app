...
export function renderPage(address, days, dateData) {
  ...
  for (let i = 0; i < days.length; i++) {
    ...
    const firstIcon = getIcon(conditions[0]);
    const firstIconsClass = conditions[0].toLowerCase().trim().replace(' ', '-');
    let secondIcon = '';
    let secondIconsClass = '';
    
    if (conditions.length !== 1) {
      secondIcon = getIcon(conditions[1].trim());
      secondIconsClass = conditions[1].toLowerCase().trim().replace(' ', '-');
    }
    
    if (days.length <= 15) {
      dateData = days[i].datetime.replace(/-/g, ', ');
    }

    const tempMax = Math.floor((days[i].tempmax - 32) * 5 / 9);
    const tempMin = Math.floor((days[i].tempmin - 32) * 5 / 9);
    const precipProb = Math.round(days[i].precipprob);
    let date = '';

    if (days.length === 2) {
      date = format(new Date(dateData), 'LLL d');
      i === 0 ? date = 'Today ' + date : date = 'Tomorrow ' + date;

      if (conditions.length === 1) {
        weatherHTML += `
          <div class="one-day-container">
            <p class="one-day-date">${date}</p>
      ...
      if (i === 0) {
        date = format(new Date(dateData[0]), 'd');
      } else if (hour === 0) {
        dayCounter += 1;
